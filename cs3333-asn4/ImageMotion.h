 ...
 *
 * All functions are declared in the header due to template usage.
 *
 * \date 17 Nov 2014
 * \author Kevin Brightwell
 */

// use a namespace to hide functions
namespace _kbright2 {
...
void findMotionLines(const cv::Mat &I, cv::Mat &out) {
    CV_Assert(dWidth % 2 == 1 && nWidth % 2 == 1); // make sure they are odd
    ...
    // okay to use in a header, it's scoped inside the function.
    using std::vector;
 ...
 */
template <int neighbourhoodWidth, int derivWidth>
void imageMotion(const std::vector<cv::Mat> &images, const cv::Vec<float, derivWidth> &deriv, cv::Mat &output) {
    ...
    cv::Mat I;
    _kbright2::calculateI(pyrImgs, deriv, I);

    cv::Mat pyrOut;
    _kbright2::findMotionLines<derivWidth, neighbourhoodWidth>(I, pyrOut);
